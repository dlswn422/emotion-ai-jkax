...
4. 동일 키워드는 positive / negative에 동시에 포함될 수 없다.

   예:
   - "부동산"이 negative에 들어갔다면
     positive에는 절대 넣지 마라.

5. 어떤 키워드를 넣을지 애매하면:
   → 등장 빈도가 더 높은 감성 쪽에만 넣어라

==============================
감성 기준 (핵심 수정)
==============================

6. 감성은 "단어 자체"가 아니라 "등장 문맥" 기준이다.
...
👉 매우 중요:
- "대통령", "부동산", "정책" 같은 중립 단어도
  긍정 문장에서 많이 등장하면 positive에 넣고,
  부정 문장에서 많이 등장하면 negative에 넣어라.

==============================
키워드 형태 규칙
==============================

7. 반드시 명사 또는 명사구 (2~10자)

가능:
- 대통령, 이재명, 부동산, 전세, 월세, 아파트, 세금, 삼성SDI
...
8. 하나의 주제는 반드시 파생 확장하라

예:

부동산 →
- 부동산, 아파트, 주택, 전세, 월세, 집값, 매매, 임대
