...
==============================
🔟 키워드 워드클라우드 (최종 최적화 버전)
==============================

목적
- 리뷰에서 반복 등장하는 핵심 키워드를 긍정 / 부정으로 나눠
  UI 워드클라우드에 바로 사용할 수 있게 풍부하게 생성한다.
...
4. 키워드 형태 규칙
   ...
   예:
   - 가능: 대통령, 이재명, 부동산, 전세, 월세, 아파트, 세금, 삼성SDI
   - 금지: "이재명이 잘함", "부동산이 문제다"

5. 인물 / 조직 / 지역 키워드 허용 (중요)
   - 정치인, 기업, 지역명 모두 포함 가능
   예:
   - 인물: 대통령, 이재명
   - 조직: 민주당, 국민의힘, 삼성SDI, LG엔솔
   - 지역: 서울

6. 감성 분류 기준 (핵심)

   positive_keywords:
   - 칭찬 / 긍정 평가 / 지지 / 호감 기반 키워드
   - 또는 긍정 맥락에서 자주 등장한 대상

   negative_keywords:
   - 비판 / 불만 / 문제 제기 / 부담 / 갈등 기반 키워드
   - 또는 부정 맥락에서 반복 등장한 대상

   중요:
   - 같은 키워드라도 맥락에 따라 긍정/부정 둘 다 포함 가능

7. 확장 규칙 (이거 안 하면 개수 부족함)

   하나의 주제를 여러 파생 키워드로 확장하라

   예:
   부동산 →
   부동산, 아파트, 주택, 전세, 월세, 집값, 매매, 임대, 공급

   정치 →
   대통령, 이재명, 민주당, 국민의힘, 선거, 정책, 공약
...
8. 불용어 제거 (매우 중요)
   ...
   아래 단어는 절대 포함 금지:
   - 감탄/평가: 최고, 별로, 좋음, 나쁨, 그냥, 진짜
   - 일반어: 문제, 부분, 상황, 경우, 느낌
   - 추상어: 경험, 요소

   👉 반드시 "의미 있는 실제 키워드"만 사용
