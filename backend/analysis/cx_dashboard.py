...
⚠️ 중요: 이 섹션은 실패 없이 반드시 조건을 만족해야 한다.

1. 리뷰 텍스트에서 반복 등장하는 핵심 단어를 최대한 많이 추출하라.
2. 반드시 positive_keywords, negative_keywords, neutral_keywords, all_keywords 4개 배열을 반환하라.

🔥 강제 조건 (반드시 만족)
- all_keywords 최소 20개 이상 (데이터 충분하면 25개 이상 권장)
- positive_keywords 최소 5개 이상
- negative_keywords 최소 5개 이상
- neutral_keywords 최소 5개 이상
- 조건을 만족하지 못하면 결과를 다시 생성하라

3. 각 키워드는 반드시 짧은 명사 또는 명사구 형태로 작성하라.
4. 불용어(예: 이, 그, 저, 것, 수, 등, 너무, 진짜 등) 절대 포함하지 마라.
5. 의미 없는 일반 단어 대신 실제 리뷰에 등장하는 구체 단어를 사용하라.
6. 키워드는 절대 요약하지 말고 가능한 한 다양하게 확장하라.

🔥 확장 규칙 (매우 중요)
- 하나의 개념에서 파생 키워드를 적극적으로 확장하라

7. all_keywords는 핵심 키워드 + 파생 키워드를 모두 포함해야 한다.
8. neutral_keywords에는 주제 키워드도 포함 가능하다.
