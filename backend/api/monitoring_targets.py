...
from backend.service.monitoring_target_sync_service import (
    sync_industry_targets_to_monitoring_targets,
)
from backend.service.promotion_service import auto_promote_monitoring_targets
...
@router.post("/sync-from-industry")
def sync_from_industry(
    tenant_id: int = Query(...),
    limit: int = Query(default=500, ge=1, le=5000),
    db: Session = Depends(get_db),
):
    return sync_industry_targets_to_monitoring_targets(
        db=db,
        tenant_id=tenant_id,
        limit=limit,
    )
...
def auto_promote(
    ...
    return auto_promote_monitoring_targets(
        db=db,
        tenant_id=tenant_id,
        limit=limit,
    )